...
pub fn build_profile(track_points: Vec<RawPoint>) -> Vec<ProfilePoint> {
    ...
    for p in track_points.iter().skip(1) {
        ...
        profile.push(ProfilePoint {
            ...
            ele: p
                .ele
                .unwrap_or(profile.last().map(|x| x.ele).unwrap_or(0.0)),
            lat: p.lat,
            lon: p.lon,
        });
        last = p;
...
pub fn render_climb_stats_to_bytes(climbs: &[Climb]) -> Result<Vec<u8>> {
    ...
    {
        let root = BitMapBackend::with_buffer(&mut pixel_buf, (img_w, img_h)).into_drawing_area();
        root.fill(&WHITE)?;
        ...
        root.draw(&PathElement::new(
            vec![(pad, summary_y - 4), (img_w as i32 - pad, summary_y - 4)],
            RGBAColor(160, 160, 160, 0.5),
        ))?;
...
fn glyph_5x7(ch: char) -> [u8; 7] {
    match ch {
        ...
        'g' | 'G' => [
            0b01110, 0b10001, 0b10000, 0b10110, 0b10001, 0b10001, 0b01110,
        ],
        'h' | 'H' => [
